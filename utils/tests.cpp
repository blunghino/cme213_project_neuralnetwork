...
void createMATS(double *A, double *B, double *C1, double *C2, int NI, int NJ, int NK)
{
    ...
    for (i = 0; i < NI; i++)
    {
        for (j = 0; j < NJ; j++)
        {
            C1[i*NJ + j] = ((double) i*j + 2) / NJ;
            C2[i*NJ + j] = ((double) i*j + 2) / NJ;
        }
    }
}
...
int compareGEMMResults(double* myC, double* refC, int NI, int NJ)
{
    ...
    // Compare myC and refC
    for (i=0; i < NI; i++) 
    {
        for (j=0; j < NJ; j++) 
        {
            if ( abs(myC[i*NJ + j] - refC[i*NJ + j]) > 1e-4 ) 
                fail++;
        }
    }
    ...
    // Print results
    if(fail) {
        std::cout << "My GEMM output not matching with reference. Num locations = " 
            << fail << std::endl;
    }
    else {
        std::cout << "GEMM matched with reference successfully!" << std::endl;
    }
