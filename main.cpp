...
int main (int argc, char *argv[]) {
	...
	while ((option = getopt(argc, argv, "n:r:l:e:b:g:sd")) != -1) {
		switch (option) {
			...
			case 'g': grade = atoi(optarg); break;
			case 'p': print_every = atoi(optarg); break;
	...
	if ((rank == 0) && (run_seq)) {		
    	...
    	std::cout << "Precision on validation set for sequential training = " << prec << std::endl;
    }
    ...
    if (rank == 0) {
    	...
    	double prec = precision (label_dev_pred, label_dev);
    	std::cout << "Precision on validation set for parallel training = " << prec << std::endl;
    ...
    if((grade || debug) && rank == 0) {
    	std::cout << std::endl << "Grading mode on. Checking for correctness" << std::endl;
